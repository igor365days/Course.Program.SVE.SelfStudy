...
## 14. Reassessment уже пройденного контроля

`REASSESSMENT` применяется только к LCP или CP, которые ранее уже имели подтверждённый успешный результат `PASSED`.

Reassessment сохраняется как новая попытка в том же файле `LCP-XX.yaml` или `CP-XX.yaml`. Предыдущие попытки не изменяются.
...
Для явной связи с исходным успешным результатом используется `recheck.previous_assessment_id`, если это поле применимо к данной попытке; связь не должна достигаться переписыванием исторической попытки.

Валидация должна различать обычную повторную проверку непройденного контроля и Reassessment уже пройденного контроля.

## 15. Инвариант исторического результата
...
не является само по себе противоречием, если Attempt #2 является `REASSESSMENT`.

Исторический `PASSED` сохраняется, а обычный маршрут остаётся основанным на ранее подтверждённом успешном прохождении, пока отдельное каноническое правило явно не задаёт иное последствие.
