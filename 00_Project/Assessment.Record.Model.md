...
Связь с исходным успешным результатом должна фиксироваться средствами, предусмотренными стандартом Assessment Record, прежде всего через `previous_assessment_id`, когда это применимо. Нельзя достигать этой связи переписыванием исторической попытки.

Результат Reassessment должен оставаться обычным `PASSED` или `FAILED`; отдельные служебные значения для Reassessment не вводятся этим документом.
