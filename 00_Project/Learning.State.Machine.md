...
Каждая обычная попытка LCP сохраняется отдельным Assessment Record внутри файла соответствующей LCP. Предыдущая попытка не удаляется.

Если LCP уже имела подтверждённый `PASSED`, последующая попытка является `REASSESSMENT`. Отрицательный результат `REASSESSMENT` не переводит уже пройденный LCP обратно в непройденное состояние и не удаляет его из `completed.lcps`. Последствия для маршрута могут быть изменены только отдельным каноническим правилом.
...
Если CP уже имел подтверждённый `PASSED`, последующая попытка рассматривается как `REASSESSMENT`. Она не отменяет историческое успешное прохождение и не удаляет CP из `completed.cps` без отдельного канонического правила.
...
- удаление ранее подтверждённого `PASSED` из `completed.lcps` или `completed.cps` только из-за отрицательного результата `REASSESSMENT`.
...
При Validation последовательность `PASSED → REASSESSMENT → FAILED` не считается противоречием, если попытка явно относится к уже пройденному контролю и не существует отдельного канонического правила, отменяющего исходный результат.
...
11. Отрицательный `REASSESSMENT` не отменяет исторический `PASSED` без отдельного канонического правила.
...
Для результата `REASSESSMENT` дополнительно проверяется, был ли соответствующий контроль ранее успешно закрыт.
...
`Assessment.Reassessment.Policy.md` определяет семантику повторной оценки уже пройденного контроля.
...
## 20. Reassessment уже пройденного контроля

`REASSESSMENT` не является обычным переходом основной State Machine для открытия или закрытия ещё не пройденного объекта.

Он применяется только после исторически подтверждённого `PASSED`.
...
PASSED
   ...
   ▼
REASSESSMENT
   ├── PASSED → исторический PASSED сохраняется
   └── FAILED → исторический PASSED сохраняется
```

Обычная отрицательная ветка непройденного контроля по-прежнему использует:

```text
FAILED → REMEDIATION → RECHECK
```

Reassessment не может использоваться для обхода этой обязательной ветки.
